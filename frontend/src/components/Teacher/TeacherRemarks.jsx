...
const TeacherRemarks = ({ selectedStudent }) => {
  ...
  const fetchExistingRemarks = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/teacher/remarks/${selectedStudent._id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `http://localhost:5000/api/teacher/remarks/${selectedStudent._id}`,
        {
  ...
  const handleDeleteRemark = async (index) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/teacher/remarks/${selectedStudent._id}/${index}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

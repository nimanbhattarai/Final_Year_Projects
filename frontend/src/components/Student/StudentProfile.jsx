        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="space-y-4">
              {Object.entries(profile.performance.academic || {}).map(([year, semesters]) => (
                <div key={year} className="border rounded-lg p-4">
                  <h3 className="font-medium text-gray-900 mb-2">Year {year}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(semesters).map(([semester, subjects]) => (
                      <div key={semester} className="bg-gray-50 p-3 rounded">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profile.performance.extracurricular.map((activity, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
                  <h3 className="font-medium text-gray-900">{activity.activity}</h3>
                  <p className="text-gray-600 mt-1">Grade: {activity.grade}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {profile.performance.teacherRemarks.map((remark, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
